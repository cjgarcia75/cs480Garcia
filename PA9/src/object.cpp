...
{ 
/*
  GLint temp = shader->GetUniformLocation("light.position");
  glUniform3f(temp, 0, 0, 0); // these zeros need to be the position of the ball
  temp = shader->GetUniformLocation("light.direction");
  glUniform3f(temp, 0, 0, -1.0);
  temp = shader->GetUniformLocation("light.cutOff");
  glUniform1f(temp, glm::cos(glm::radians(12.5f)));
  temp = shader->GetUniformLocation("viewPos");
  glUniform3f(temp, 0, 0, 0);
  temp = shader->GetUniformLocation("light.ambient");
  glUniform3f(temp, 0.1f, 0.1f, 0.1f);
  temp = shader->GetUniformLocation("light.diffuse");
  glUniform3f(temp, 0.8f, 0.8f, 0.8f);
  temp = shader->GetUniformLocation("light.specular");
  glUniform3f(temp, 1.0f, 1.0f, 1.0f);
  temp = shader->GetUniformLocation("light.constant");
  glUniform1f(temp, 1.0f);
  temp = shader->GetUniformLocation("light.linear");
  glUniform1f(temp, 0.09f);
  temp = shader->GetUniformLocation("light.quadratic");
