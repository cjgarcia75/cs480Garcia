...
bool Object::LoadObjFile(std::string objFile)
{
  ...
  const aiScene* myScene = importer.ReadFile(objFile, aiProcess_Triangulate);
  std::cout << importer.GetErrorString() << "\n";
  if(myScene == NULL)
  {
    std::cout << "myScene is null\n";
    return false;
  }
  ...
  for(int i = 0; i < mesh->mNumFaces; i++)
  {
    ...
    for(int vertexNum = 0; vertexNum < face->mNumIndices; vertexNum++)
    {
      ...
      Vertex tempVert(glm::vec3(tempPos.x, tempPos.y, tempPos.z), glm::vec3(colors()));
