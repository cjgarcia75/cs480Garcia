...
void Object::Update(unsigned int dt)
{
  ...
  {
    model = glm::translate(glm::mat4(1.0f), glm::vec3((3 * cos(angle)), -2.0, (3 * sin(angle))));
    tranModel = model;
  }
...
bool Object::LoadObjFile(std::string objFile)
{
  bool Ret = false;
  ...
  if (pScene) 
  {
    Ret = InitFromScene(pScene, objFile);
  }
  else 
  {
...
bool Object::InitFromScene(const aiScene* pScene, const std::string& objFile)
{  
  m_Entries.resize(pScene->mNumMeshes);
  m_Textures.resize(pScene->mNumMaterials);
  
  //std::cout << "num meshes: " << pScene->mNumMeshes << "\n";
  ...
  return InitMaterials(pScene, objFile);
}
...
bool Object::InitMaterials(const aiScene* pScene, const std::string& objFile)
{
  ...
  for (unsigned int i = 0 ; i < pScene->mNumMaterials ; i++) 
  {
    ...
    if (!m_Textures[i]) 
    {
      m_Textures[i] = new Texture(GL_TEXTURE_2D, "../assets/earth.jpeg");
